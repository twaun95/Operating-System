...
secondtable table2[0x3FF];
Directory directory[MAXPROC];
int main()
{
...
void make_table()
{
	...
	for(int k=0; k<NUM; k++)
	{
		...
		tableindex = (PCB[ptr].VA[k] & 0x3FF000) >> 12;
		...
		//check hit
		for(int k=0; k<64; k++){
			if((tlb[k].full == 1) && (tlb[k].pn == tableindex)){
				HIT = 1;
				tlbindex = k;
			}
		}
		...
		if(HIT){
			...
			Address = (tlb[tlbindex].pfn << 12) | offset;
			tlb[tlbindex].sca = 1;
			printf("HIT!!, Tick %d = VA[%d] : 0X%08X -------- Address : 0X%08X  tlbindex = %d\n", tick, k, PCB[ptr].VA[k], Address,tlbindex);
		}

		else if(!HIT){
			if(directory[TTBR].valid[dirindex] == 0){ //notintable
				 directory[TTBR].dn[dirindex] = Dequeue(qfree) & 0xFFC00000 >> 22;
		
				if(table2[directory[TTBR].dn[dirindex]].valid[tableindex] == 0){
					table2[directory[TTBR].dn[dirindex]].pfn[tableindex] = Dequeue(qfree) & 0xFFFFF000 >> 12;
					Address = (table2[directory[TTBR].dn[dirindex]].pfn[tableindex] << 12) | offset;
				//tlb[i].pn = dirindex;
				//tlb[i].pfn = framenum;
				//tlb[i].full = 1;
				//printf("tlb[%d].pfn = 0X%08X\n", i, tlb[i].pfn);
				//i = (i+1)%64;
					table2[directory[TTBR].dn[dirindex]].valid[tableindex] = 1;
					printf("Tick %d = VA[%d] : 0X%08X -------- Address : 0X%08X\n", tick, k, PCB[ptr].VA[k], Address);	
				}

				directory[TTBR].valid[dirindex] = 1;	
			}

			else if(directory[TTBR].valid[dirindex] == 1){
				if(table2[directory[TTBR].dn[dirindex]].valid[tableindex] == 0){
					table2[directory[TTBR].dn[dirindex]].pfn[tableindex] = Dequeue(qfree) & 0xFFFFF000 >> 12;
					Address = (table2[directory[TTBR].dn[dirindex]].pfn[tableindex] << 12) | offset;
					table2[directory[TTBR].dn[dirindex]].valid[tableindex] = 1;
				...
				else if(table2[directory[TTBR].dn[dirindex]].valid[tableindex] == 1){
					Address = (table2[directory[TTBR].dn[dirindex]].pfn[tableindex] << 12) | offset;
					tlb[i].pn = tableindex;
					tlb[i].pfn = table2[directory[TTBR].dn[dirindex]].pfn[tableindex];
					tlb[i].full = 1;
					i++;
					if(i == 30){
						i=0;
...
/*
	//mapping
	//check tlbfull(check all tlb)
		//for(int i=0; i<64; i++)
		//full X 
			//put tlb
		//full o(eviction)--LRU or sca
			//check sca
			//put tlb
	//if(directory[TTBR].valid[dirindex] == 1)
	//check tlbfull(check all tlb)
		//full x
			//put tlb
		//full o(eviction)--LRU or sca
