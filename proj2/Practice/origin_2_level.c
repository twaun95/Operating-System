...
void make_table()
{
	...
	for(int k=0; k<NUM; k++)
	{
		...
		if(directory[TTBR].valid[dirindex] == 0)
		{
		...
		{
			if(table2[directory[TTBR].dn[dirindex]].valid[index] == 0)
			{
			table2[directory[TTBR].dn[dirindex]].pfn[index] = Dequeue(qfree) & 0xFFFFF000 >> 12;
				table2[directory[TTBR].dn[dirindex]].valid[index] = 1;
			}
			
			if(table2[directory[TTBR].dn[dirindex]].valid[index] == 1)
			{
				Address = (table2[directory[TTBR].dn[dirindex]].pfn[index] << 12) | offset;
