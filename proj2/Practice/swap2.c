	...
	for(int k=0; k<NUM; k++)
	{
		...
		if(dir_valid == 0x0)
		{
			...
			if(blocktable[PCB[ptr].TTBR][index].where != 0x0)
                        {
                                memcpy(&pageframe[directory[PCB[ptr].TTBR][dirindex].dn >> 12].data, &block[blocktable[PCB[ptr].TTBR][dirindex].where].data, sizeof(block[blocktable[PCB[ptr].TTBR][dirindex].where].data));
                        }
			else
			{
				pageframe[(directory[PCB[ptr].TTBR][dirindex].dn) >> 12].data[offset] = table2[(directory[PCB[ptr].TTBR][dirindex].dn) >> 12][index].pfn;
			}
		}
		if(dir_valid == 0x1)
		{
				...
				if(blocktable[(directory[PCB[ptr].TTBR][dirindex].dn) >> 22][index].where != 0x0)
                        	{
                                	...
                                	printf("swapin %x Tick %d, TTBR %d = VA[%d] : 0X%08X -------- pageframe[0X%05X] = %d\n", blocktable[(directory[PCB[ptr].TTBR][dirindex].dn) >> 22][index].where, tick, PCB[ptr].TTBR, k, PCB[ptr].VA[k], table2[(directory[PCB[ptr].TTBR][dirindex].dn) >> 22][index].pfn >> 12, pageframe[table2[PCB[ptr].TTBR][index].pfn >> 12].data[offset]);
                        	}
                        	else
                        	{
