...
void make_table()
{
	...
	for(int k=0; k<NUM; k++)
	{
		...
		HIT=0;
		for(int k=0; k<30; k++){
			if((tlb[k].full == 1)&&(tlb[k].pn == index)){
				HIT = 1;
				tlbindex = k;
			}
		}
		...
		//	tlb[tlbindex].sca = 1;		
			...
			if(pagetable[TTBR].valid[index] == 0)
			{
				pagetable[TTBR].pfn[index] = Dequeue(qfree) & 0xFFFFF000 >> 12;
				Address = (pagetable[TTBR].pfn[index] << 12) | offset;	
				pagetable[TTBR].valid[index] = 1;
				printf("Tick %d, TTBR %d = VA[%d] : 0X%08X -------- Address : 0X%08X\n", tick, PCB[ptr].TTBR, k, PCB[ptr].VA[k], Address);
			}	

			else if(pagetable[TTBR].valid[index] == 1)
			{
				Address = (pagetable[TTBR].pfn[index] << 12) | offset;
				tlb[i].pn = index;
				tlb[i].pfn = pagetable[TTBR].pfn[index];
				tlb[i].full = 1;
				i++;
				if(i==30){
					i=0;
